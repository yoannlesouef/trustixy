...
**Canonical disclaimer text (v1.0) — single source of truth:**
...
`disclaimer_version = "1.0"` stored on every `documents` row. This text is embedded verbatim in every PDF and in the `documents.content` field. It is non-removable. If the text changes, increment the version and create a new document row — never modify existing documents.
